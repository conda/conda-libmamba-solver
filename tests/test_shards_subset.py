...
from .test_shards import CONDA_FORGE_WITH_SHARDS, channels_to_dict
...
def test_traversal_algorithms_match(conda_cli, scenario: dict):
    ...
    repodata_algorithm_map = {
        "bfs": build_repodata_subset(
            scenario["packages"], channels_to_dict([channel], True), algorithm="bfs"
        ),
        "pipelined": build_repodata_subset(
            scenario["packages"], channels_to_dict([channel], True), algorithm="pipelined"
        ),
    }
...
def test_build_repodata_subset_package_not_found(http_server_shards, algorithm, tmp_path, mocker):
    ...
    channel_data = build_repodata_subset(
        root_packages, channels_to_dict([channel], True), algorithm=algorithm
    )
...
def test_only_tar_bz2(http_server_shards, tmp_path, only_tar_bz2, strategy):
    ...
    channel_data = fetch_channels(channels_to_dict([channel], True))
...
def test_pipelined_with_slow_queue_operations(http_server_shards, mocker, tmp_path):
    ...
    channel_data = build_repodata_subset(
        root_packages, channels_to_dict([channel], True), algorithm="pipelined"
    )
...
def test_pipelined_timeout(http_server_shards, monkeypatch, tmp_path):
    ...
    # shardlikes = fetch_channels(expand_channel([channel], True))
